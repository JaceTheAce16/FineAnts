-- ==============================================================================
-- FINEANANTS SAFEST DATABASE SETUP
...
-- Make event_id NOT NULL after adding it
DO $$ BEGIN
  ALTER TABLE webhook_events ALTER COLUMN provider SET NOT NULL;
  ALTER TABLE webhook_events ALTER COLUMN event_id SET NOT NULL;
  ALTER TABLE webhook_events ALTER COLUMN event_type SET NOT NULL;
EXCEPTION
  WHEN others THEN NULL;
END $$;

-- Add UNIQUE constraint to event_id if it doesn't exist
DO $$ BEGIN
...
-- Add sync status columns to plaid_items if they don't exist
DO $$ BEGIN
...
-- Create conditional index for sync status
DO $$ BEGIN
